...
def glob_mem_temp(mem_num, global_spatial_length, global_temporal_length):
    MEM_ADDR_MIN = 0x00500000               
    MEM_ADDR_MAX = 0xFF539000
    ...
    min_loop_inst = 2 ** (global_temporal_length - 1) + 1
    if mem_num - 2 > min_loop_inst:
        loop_inst_num = min_loop_inst    # 单次 “循环” 中的指令数量，loop_inst_num 是 2 的倍数
    else:
        raise NotFoundError              # 不满足一次循环

    if loop_inst_num * (8 ** (global_spatial_length - 1)) > MEM_SIZE:       # 单次循环中，访问最大的地址空间
        raise NotFoundError                                                 # 不满足空间分布

    # generate the distribution
    distribution = []
    for i in range(int((mem_num) / loop_inst_num) * loop_inst_num):
        if int(i % loop_inst_num) == 0:
...
def inst_allocate(inst_type, inst_mix, block_inst, global_spatial_length, global_temporal_length):
    ...
    if global_spatial_length != 0:
        ...
        if global_spatial_length <= 4:
            ...
            block_inst[load_inst_index[0]] = ['mov', imme_reg, '#' + str(hex(load_offset_imme))]
            inst_mov_num += 1
    else:
        load_offset_imme = 0
    ...
    inst_mix['int_alu_inst_num'] = inst_mix['int_alu_inst_num'] - inst_mov_num
